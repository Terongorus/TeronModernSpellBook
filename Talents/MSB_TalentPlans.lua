--[[
	...
	ModernSpellBook_DB.talentPlans; CTalentIcon/CTalentGrid read it the same way they read real
	GetTalentInfo/GetTalentTabInfo data, via TalentTree.mode switching which source is "current".
--]]

local MAX_PLAN_COUNT = 20
local CHAT_PREFIX = "|cff00ff00ModernSpellBook:|r "
...
class "CTalentPlanService"
{
	...
	EnsureDB = function(self)
		if (not ModernSpellBook_DB) then
			...
			-- ModernSpellBook_DB's SavedVariables are actually injected. Return a throwaway
			-- placeholder instead of crashing; the dropdown always re-initializes again right
			-- before it's actually shown to the user, by which point ModernSpellBook_DB is
		...
		if (not ModernSpellBook_DB.talentPlans) then
			ModernSpellBook_DB.talentPlans = { selectedPlan = 1, plans = {} }
		end
		return ModernSpellBook_DB.talentPlans
	end;
