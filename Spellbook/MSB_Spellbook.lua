...
class "CSpellBook"
{
	...
	OnAddonLoaded = function(self)
		if (arg1 ~= "TeronModernSpellBook") then return end
	...
	SetShape = function(self, isMainFrameMinimized)
		-- Legacy WhatsTraining overlay: only when the old WT UI created its Blizzard
		-- frame. New WT versions (folder/TOC: TeronWhatsTraining) register a native MSB
		-- tab and never create it.
		if (IsAddOnLoaded and IsAddOnLoaded("TeronWhatsTraining") and WhatsTrainingFrame) then
	...
	RefreshPageElements = function(self)
		...
		if (self.frame.maxPages > 1) then
			self.frame.pageText:SetText("Page " .. currentPage .. " / " .. self.frame.maxPages)
